 ...
 */
public class ModificarSocioServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
      ...
      //   socio.setCedula(Integer.parseInt(cedula));
         socio.setNombre(nombre);
         socio.setApellido(apellido);
         UsuarioDAO managerUsuario = new UsuarioDAO();
         int resultado = managerUsuario.editarSocio(socio);
         if(resultado == 1){
             url = "./PrincipalAdmin.jsp?listarSocios";
                response.setHeader("Location", url);
                response.sendRedirect(url);
         }else{
             url = "./PrincipalAdmin.jsp?errorModificarSocio";
                response.setHeader("Location", url);
                response.sendRedirect(url);
         }
    }
